...
function domGenerate() {
    for (let index = 0; index < semestres.length; index++) {
        let container = document.getElementsByClassName('semestres')[0];
        let element = document.createElement('div');
        element.className = 'semestre';
        element.id = 'semestre-' + semestres[index].id_semestre;
        element.innerHTML = `
            <h2>Semestre: ${semestres[index].id_semestre}</h2>
            <h3>Creditos Totales: ${semestres[index].creditos_totales}</h3>
        `;
        let semestreId = index;

        container.appendChild(element);
        domModulos(semestreId);
    }
}


function domModulos(semestreId) {
    for (let index = 0; index < semestres[semestreId].modulos.length; index++) {
        const semestreTagId = 'semestre-';
        let container = document.getElementById(semestreTagId.concat(semestres[semestreId].id_semestre));
        let element = document.createElement('div');
        element.className = 'modulo modulo-' + semestres[semestreId].modulos[index].tipo_modulo;
        element.id = 'modulo-' + semestres[semestreId].modulos[index].id_modulo;

        element.innerHTML = `
            <p>Modulo: ${semestres[semestreId].modulos[index].nombre_modulo}</p>
            <span>Creditos por Modulo: ${semestres[semestreId].modulos[index].creditos_modulo}</span>
        ...
        domMaterias(semestreId, moduloId);
    }
}

function domMaterias(semestreId, moduloId) {
    for (let index = 0; index < semestres[semestreId].modulos[moduloId].materias.length; index++) {
        const moduloTagId = 'modulo-';
        let container = document.getElementById(moduloTagId.concat(semestres[semestreId].modulos[moduloId].id_modulo));
        let element = document.createElement('div');
        element.className = 'materia'
        element.id = 'materia-' + semestres[semestreId].modulos[moduloId].materias[index].id_materia;
        element.innerHTML = `
                <span class='clave-materia'>${semestres[semestreId].modulos[moduloId].materias[index].clave_materia}</span>
                <span class='nombre-materia'>${semestres[semestreId].modulos[moduloId].materias[index].nombre_materia}</span>
                <span class="creditos-materia">${semestres[semestreId].modulos[moduloId].materias[index].creditos_materia}</span>
                <span class="periodo-aprobada">${semestres[semestreId].modulos[moduloId].materias[index].periodo_aprobada}</span>
                `;
        let materiaId = index;
        container.appendChild(element);
        domMateriaStauts(semestreId, moduloId, materiaId);
...
function domMateriaStauts(semestreId, moduloId, materiaId) {
    const materiaTagId = 'materia-';
    let container = document.getElementById(materiaTagId.concat(semestres[semestreId].modulos[moduloId].materias[materiaId].id_materia));
    let element = document.createElement('div');
    element.className = 'icon status-icon'

    if (semestres[semestreId].modulos[moduloId].materias[materiaId].status == 'aprobada') {
        element.innerHTML = `
            <i class='bx bx-check'></i>
        `;
    }
    else if (semestres[semestreId].modulos[moduloId].materias[materiaId].status == 'reprobada') {
        element.innerHTML = `
            <i class='bx bx-x'></i>
        `;
    }
